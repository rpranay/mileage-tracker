        ...
        elevation = CardDefaults.cardElevation(defaultElevation = 4.dp)) {
          if (mileageEntries.value.isNotEmpty()) {
            ...
            Box(modifier = Modifier.fillMaxSize(), contentAlignment = Alignment.Center) {
              Text("Add entries to see chart", color = Color.Gray, fontSize = 16.sp)
            }
          }
        }
        ...
        Row(
            ...
            verticalAlignment = Alignment.CenterVertically) {
              Column(modifier = Modifier.weight(1f)) {
                ...
                Text(text = dateFormat.format(entry.date), fontSize = 14.sp, color = Color.Gray)
              }
...
@Composable
fun MileageChart(entries: List<MileageEntry>, modifier: Modifier = Modifier) {
  ...
  Canvas(modifier = modifier.fillMaxSize().padding(16.dp)) {
    ...
    sortedEntries.forEachIndexed { index, entry ->
      ...
      val y =
          height -
              padding -
              ((entry.miles - minMiles).toFloat() / mileRange) * (height - 2 * padding)
